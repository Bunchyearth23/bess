...
2. **Generate sound in BESS:** New imports measure the Automation exhaust bank to guide an independent pulse, flow, intake and mechanical model. Adjust its character and compare the result with the original reference in live playback, save a project or render WAVs. Existing projects retain their saved sound mode.
...
BESS is built with Rust, egui and BDSP. Automation's exhaust bank supplies measured level and broad spectral characteristics. The generated mode creates its own waveform rather than playing Automation PCM through effects. Automation exports only exhaust audio, so the engine-side and intake detail remain estimates rather than recovered recordings.
...
6. Try **Balanced**, **Muted**, **Open**, **Warm**, **Mechanical** and **Grit**. These presets change tone and generated source balance while preserving engine timing and driving response. Fine-tune exhaust body, edge, flow texture and mechanical detail in the sound settings.

B uses independently generated excitation guided by descriptors measured from the imported bank. A remains the prepared Automation reference. The earlier source-guided sound mode and its **Cycle variation**, **Pressure front**, **Pulse-linked texture** and **Resynthesis amount** controls remain available for existing projects. Neither mode measures cylinder pressure or intake flow.
...
The portable v0.10.0 package contains the Windows application and its instructions. It does not include test vehicles, sound banks, sample projects, or prepared BeamNG add-ons. Import your own Automation vehicle ZIP into BESS, then export a matching BESS configuration add-on. Keep your original vehicle ZIP enabled beside that add-on in BeamNG. Select the trim marked **(BESS)**; the original trim remains available.

New imports use the generated model by default. Projects saved with the previous source-guided mode retain that mode. The two-emitter preview and BeamNG level panel estimate exported file levels; final loudness and naturalness need in-game listening.
...
The 0.10.0 portable package contains no example vehicles or listening corpus. Use your own Automation exports for A/B listening and BeamNG configurations. Measurements describe signal differences; only listening can judge naturalness.
...
- `cargo run --release -- --beamng vehicle.zip new-folder`: configuration add-on with generated BESS loops; keep the original mod enabled.
