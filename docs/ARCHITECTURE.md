...
`export` prepares stable loops per rpm and load after one second of audio-engine warm-up, with cyclic overlap and common safety gain. It changes only the referenced engine WAVs and the top-level `Name` in `vehicles/<model>/info.json`, appending `(BESS)` for the vehicle selector. Other ZIP entries remain byte-identical. The narrow metadata edit preserves duplicate paint keys in Automation exports. Export verifies the source, rejects duplicates/suspicious paths, does not overwrite an existing folder and reimports the output archive before finalizing it. Turbo, pops and driving transients are not baked into loops; game references remain intact. The manifest distinguishes technical delivery from in-game validation. Each ZIP name includes the original archive name and a short hash, but the original and BESS copy still share internal paths and must not be active together. Tone presets preserve explicit combustion configuration.
