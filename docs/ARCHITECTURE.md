...
`export` prepares stable loops per rpm and load after at least one second of audio-engine warm-up, ending at a common 720-degree phase for every RPM knot. Cyclic overlap joins each loop, and one safety gain covers the exhaust bank. `Hybrid::next_stems` keeps the normal audition mix unchanged, removes its small intake/mechanical component from the exhaust export stem, and produces a distinct engine-side proxy. That proxy uses the high-passed source residual with a one-cycle difference to suppress leaked exhaust orders; it adds quiet, slightly irregular impacts at fixed structural resonances when the cylinder count is verified from Automation or set explicitly. It is derived from one exhaust recording, not a measured microphone track. The engine loops last about four seconds to delay exact mechanical-pattern repetition; the exhaust loops stay near two seconds. The current GUI path builds a configuration add-on ZIP for the imported Automation vehicle. It clones the source `.pc` and `info_<config>.json`, gives the new configuration a unique filename and a `Configuration` label ending in `(BESS)`, and selects a uniquely named clone of the primary engine JBeam part. That part connects separate `soundConfig` and `soundConfigExhaust` blends with their own 48 kHz / 24-bit WAV paths. Engine gain is bounded separately at each RPM knot, with a PCM peak ceiling and a lower off-load target; the JBeam engine emitter starts 8 dB below the existing Automation exhaust emitter. The add-on does not include or overwrite the base `vehicles/<model>/info.json` or the original configuration; both ZIPs can be enabled together so the original and BESS trims remain selectable. Sibling JBeam parts, including engine management and turbo parts, continue to come from the original mod.

The legacy full-replacement export API retains the complete-vehicle ZIP workflow. It changes the referenced engine WAVs and the top-level `Name` in `vehicles/<model>/info.json`, appending `(BESS)` to the vehicle selector name. Other ZIP entries remain byte-identical. Its narrow metadata edit preserves duplicate paint keys in Automation exports. Because this full copy shares the original mod's internal paths, only one of them should be enabled at a time. The published v0.8.4 release downloads predate the GUI add-on workflow. Export verifies the source, rejects duplicates and suspicious paths, and does not overwrite an existing folder. Turbo, pops and driving transients are not baked into loops; existing game references remain intact. The manifest distinguishes technical delivery from in-game validation. Tone presets preserve explicit combustion configuration.
