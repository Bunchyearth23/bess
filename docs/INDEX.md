...
- Current correction: v0.10.1 returns new imports and every BeamNG export to source-guided synthesis. The independent generator stays in the interface for experimental listening, and the BeamNG preview and level panel follow the standard export. Twelve new standard add-ons are installed locally beside the unchanged Automation vehicles; the 0.10.0 procedural add-ons are backed up. The local application packages exclude vehicle files. The user's BeamNG listening remains open. See the [0.10.1 correction report](reports/BESS-0.10.1-STANDARD-FLEET-2026-09-23.md), [0.10.0 history](reports/BESS-0.10.0-FLEET-2026-09-23.md) and [R8 experiment](reports/BESS-PROCEDURAL-ENGINE-R8-2026-09-23.md).
...
| D-017 | accepted | BESS means Bunchy's Engine Synthesis System. Present its core purpose as the intermediary between Automation vehicle sound exports and BeamNG.drive vehicle mods: import, synthesize and preview, then export. |
| D-018 | accepted | Export a selectable BESS configuration beside the original Automation vehicle by default. Use unique engine part, blend and WAV paths; retain the older full-replacement export only as a separate legacy option. |
| D-019 | accepted | Following the in-game no-difference report, add a distinct BeamNG `soundConfig` engine/intake emitter to selectable BESS configurations. Limit the inferred engine stem per RPM knot and phase-align exported loops; retain the earlier mixed sound for explicit legacy full replacements. |
| D-020 | accepted | Following the user's R6 listening report, use Automation exhaust audio as the timing and timbre reference for stronger BESS resynthesis. Keep A as the unchanged source, reduce dry-source dominance in B, and infer engine/intake detail without claiming it was recorded separately. Judge naturalness by BeamNG listening after technical checks. |
| D-021 | accepted | Following the user's R7 listening report, prototype a separately selectable procedural B path: learn bounded level, timing and broad spectral descriptors from Automation, then generate new exhaust, intake and mechanical excitation. Preserve A and R7 for comparison; do not claim an authentic firing order or measured intake from an exhaust-only ZIP. Retain mono 48 kHz / PCM24 BeamNG delivery. |
| D-022 | superseded by D-023 | In 0.10.0, use the descriptor-guided path for new imports and retain the saved mode of old projects. Offer six sound characters with separate generated body, edge, flow and mechanical controls. Export all twelve local vehicles as selectable variants for user testing; exclude those vehicles from Git and public downloads. |
...
- [x] W-002.7 Selectable add-ons replaced the local full-copy installation after user feedback: twelve originals plus twelve `(BESS)` configurations active; 688 PCM24 loops, 748 unique add-on paths and zero overlaps with original or unrelated mods. Previous full copies remain in an inactive backup. Cerberus CLI export and 49 tests passed; see the [variant report](reports/BEAMNG-BESS-VARIANTS-2026-09-23.md). In-game sound and selector behavior remain W-002.4.
- [x] W-002.8 Dual-layer revision after user listening: all twelve add-ons now provide separate engine/intake and exhaust blends, 1,376 PCM24 loops and 1,448 non-overlapping paths. Twelve earlier exhaust-only add-ons are backed up; twelve originals are unchanged. BeamNG mounted and activated the 24 current mods. See the [dual-layer report](reports/BEAMNG-BESS-DUAL-LAYERS-2026-09-23.md). Audible balance and realism remain W-002.4 / W-003.6.
- [x] W-002.9 Cerberus follow-up: user confirmed the two-layer note is audible but called its regular drone artificial. Shared-cycle export phase and bounded per-knot engine balance improved measured continuity; a revised Cerberus ZIP passed structural/format checks and BeamNG dynamically mounted it. The other eleven variants await the Cerberus listening result; see the [follow-up report](reports/BEAMNG-BESS-DUAL-LAYERS-2026-09-23.md).
- [x] W-002.10 Cerberus R6 technical candidate after user rejected the native V6 and identified missing mechanics: continuous synthetic airflow removed; source residual decorrelated by one cycle; quiet mechanical impacts vary slightly in time and strength; engine loops extended to about four seconds. ZIP format, CRC, path isolation and installed hash verified; 51 Rust tests and strict Clippy pass. At 5,338 rpm on load, harmonic share fell from R5's 51.3% to 21.6%, and nominal-gain phase-sweep level span fell from 1.25 to 0.57 dB. User naturalness and actual BeamNG RPM-blend behavior remain open; see the [follow-up report](reports/BEAMNG-BESS-DUAL-LAYERS-2026-09-23.md).
- [x] W-002.11 Eleven additional R6 vehicle candidates rendered offline and verified alongside Cerberus: twelve source hashes, all ZIP CRCs, 1,448 disjoint paths, and 1,376 nonempty mono 48 kHz / PCM24 WAVs checked. Only Cerberus R6 is installed pending listening. Standalone variant conversion now rejects a source ZIP with a different bank fingerprint or blend, preventing mismatched engine/exhaust stems. The local preview executable is rebuilt; see the [follow-up report](reports/BEAMNG-BESS-DUAL-LAYERS-2026-09-23.md) and `output/all-vehicles-mechanical-r6-20260923/verification.json`.
- [x] W-002.12 BeamNG export level panel: an explicit background scan measures the exact PCM24 exhaust and engine file AC RMS/peaks at every original RPM/load point, including per-knot exhaust calibration, bank-wide safety gain and per-point engine gain. An independent inspection RPM slider shows BESS exhaust versus Automation and engine versus exhaust at neighboring points without recomputing the bank; export-affecting changes mark results stale. Listening volume is labeled separately. These are file levels; the game's spatial mix and final audible level remain open under W-002.4.
- [x] W-002.13 All eleven remaining R6 add-ons installed locally beside the already-current Cerberus and twelve original vehicles. Active and backup SHA-256 hashes match their local manifests; all twenty-four active ZIPs pass CRC. Public release assets exclude these test vehicles and add-ons; see the [fleet and release report](reports/BESS-0.9.0-RELEASE-2026-09-23.md). BeamNG was closed before final installation, so W-002.4 remains open.
- [x] W-002.14 R7 source-guided candidate: the one-cycle intake comb is replaced by a shifted adaptive residual; modeled pulse/acoustics, restrained mechanics and source-relative AC calibration feed separate BeamNG emitters. Twelve local comparisons and 68 release tests passed; Cerberus, Genesis and Berlingo selectable ZIPs passed focused file checks. Only Cerberus R7 replaced its R6 add-on locally; the prior ZIP is backed up, the twelve originals and other eleven add-ons retain their hashes. A no-vehicle local app preview is available; see the [R7 report](reports/BESS-SOURCE-GUIDED-RESYNTHESIS-2026-09-23.md). The user subsequently found R7 metallic and source-like in BeamNG, so sound acceptance remains W-002.4 / W-003.6.
- [x] W-002.15 R8 procedural candidate: import-time level, spectral, residual and firing-order descriptors drive independent exhaust/intake/mechanical excitation. The 80 Hz-aware export, listening preview and level panel agree on calibration. The final Cerberus candidate in `output/cerberus-r8-review-20260923` passed 77 release tests, strict Clippy, a ten-second 48 kHz device callback check, ZIP CRC/path checks and 132 mono 48 kHz/PCM24 loop checks. Original and installed R7 mods remain untouched. Cerberus full-load tone remains too mid-forward by measured bands, and user preference/in-game behavior remain open under W-002.4 / W-003.6; see the [R8 experiment](reports/BESS-PROCEDURAL-ENGINE-R8-2026-09-23.md).
- [x] W-002.16 v0.10.0 fleet: twelve generated add-ons installed locally beside twelve unchanged originals; previous add-ons backed up. All 1,448 paths are distinct from originals and each other, and all 1,376 WAVs pass mono 48 kHz / PCM24 checks. Six presets, 78 release tests, strict Clippy and application-only package verification passed. User listening is still W-002.4 / W-003.6; see the [fleet report](reports/BESS-0.10.0-FLEET-2026-09-23.md).
- [x] W-002.17 v0.10.1 correction: default imports, selectable and full-replacement BeamNG exports, two-emitter preview and level estimates use the source-guided mode. Twelve standard add-ons replaced the procedural fleet locally; twelve original vehicle hashes remain unchanged and the previous add-ons are backed up. All 1,448 add-on paths are disjoint, 1,376 WAVs pass mono 48 kHz / PCM24 checks, and each project and manifest records `procedural: false`. Eighty release tests and strict Clippy pass. User listening remains W-002.4 / W-003.6; see the [correction report](reports/BESS-0.10.1-STANDARD-FLEET-2026-09-23.md).
...
| X-008 | open | P1 | User found R6 intake artificial and R7 metallic and still source-like; an exhaust-only recording cannot identify the real engine-bay/intake sound | Evaluate R8 against installed R7 in BeamNG; separate intake/engine recordings remain the route to source-accurate detail |
| X-009 | open | P1 | R8 differs from Automation but Cerberus full-load still has too little 80–500 Hz and too much 2–5 kHz power at 5,200 rpm; Berlingo improves over the first trial but still lacks some irregular upper texture. Exported steady loops may repeat audibly. | Obtain matched-level user listening and in-game RPM/load sweeps; use the final per-knot 80 Hz-aware level report and do not treat waveform correlation as a realism score |
