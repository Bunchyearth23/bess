...
- Current technical delivery: the v0.9.0 public Windows downloads contain the application and instructions only; test vehicles, sound banks, projects, and prepared BeamNG add-ons are excluded. The twelve original Automation vehicles and twelve matching R6 add-ons remain installed only in the local BeamNG mods folder, with eleven old variants backed up outside it. Local hashes and ZIP CRCs pass, but BeamNG was closed before the final fleet installation, so mounting and listening remain to be checked. The user found the first engine layer too regular and R4's airflow synthetic; R6 has not yet received a listening verdict. See the [fleet and release report](reports/BESS-0.9.0-RELEASE-2026-09-23.md).
