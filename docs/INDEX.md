...
- Current technical delivery: the v0.9.0 Windows build and portable package contain selectable two-layer R6 variants for all twelve Automation vehicles. The active BeamNG mods folder has the twelve originals and twelve matching R6 add-ons; eleven old variants are backed up outside it. File hashes and ZIP CRCs pass, but BeamNG was closed before the final fleet installation, so mounting and listening remain to be checked on the next launch. The user found the first engine layer too regular and R4's airflow synthetic; R6 uses quieter, slightly irregular mechanical texture and a source-residual cycle difference, but has not yet received a listening verdict. See the [fleet and release report](reports/BESS-0.9.0-RELEASE-2026-09-23.md).
...
- [x] W-002.12 BeamNG export level panel: an explicit background scan measures the exact PCM24 exhaust and engine file RMS/peaks at every original RPM/load point, including the bank-wide exhaust safety gain and per-point engine gain. An independent inspection RPM slider shows BESS exhaust versus Automation and engine versus exhaust at neighboring points without recomputing the bank; export-affecting changes mark results stale. Listening volume is labeled separately. These are source-file levels; the game's spatial mix and final audible level remain open under W-002.4.
- [x] W-002.13 All eleven remaining R6 add-ons installed beside the already-current Cerberus and twelve original vehicles. Active and backup SHA-256 hashes match their manifests; all twenty-four active ZIPs pass CRC. The v0.9.0 package contains the same twelve R6 bytes, 1,376 PCM24 WAVs, and portable source/project references; see the [fleet and release report](reports/BESS-0.9.0-RELEASE-2026-09-23.md). BeamNG was closed before final installation, so W-002.4 remains open.
...
| X-008 | open | P1 | Engine-side detail inferred from only Automation's exhaust recording may retain synthetic timbre even after phase and level correction | User trial of R6 Cerberus; if still artificial, obtain independent engine/intake source evidence or further isolate the emitter |
| X-002 | open | P1 | Audio stability depends on device and buffer size | Extended real-device test |
| X-003 | open | P2 | The modeled linear crossfade of adjacent Cerberus BeamNG WAVs still dips about 2.35 dB on load near 5,200 rpm; BeamNG's actual blend law is unverified | Slow in-game sweep around 5,200 rpm; adjust blending if distracting |
