...
- Current technical delivery: the public v0.8.4 assets at `dist/BESS-0.8.4-public/START.html` predate the selectable two-layer add-ons. All twelve original Automation vehicles remain active beside twelve BESS variants in BeamNG. The user heard the first engine layer but found it too regular, then found R4's added airflow synthetic and preferred R4 over a BeamNG-native V6 reference. A Cerberus-only R6 candidate now replaces continuous added airflow with lighter, slightly irregular mechanical texture and a source-residual cycle difference; it is installed but not yet heard by the user. The other eleven variants still use the first dual-layer build. A matching local executable is at `dist/BESS-mechanical-preview-20260923/BESS.exe`; it is not a new public release. See the [dual-layer and Cerberus follow-up report](reports/BEAMNG-BESS-DUAL-LAYERS-2026-09-23.md).
